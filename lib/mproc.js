...
function Processor() {
    ...
    this.runSync = function (msg, name) {
        ...
        function doStep(msg) {
            ...
            runStep(step, msg, context, function(err, msg) { if (!err) doStep(msg); });
        }
    ...
    this.run = function (msg, name) {
        ...
        function doStep(msg) {
            if (nstep >= steps.length)
                return;
            var step = steps[nstep++];
            runStep(step, msg, context, function(err, msg) { if (!err) setImmediate(function () { doStep(msg); }); });
        }
